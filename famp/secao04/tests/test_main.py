...
class TestClass:
    ...
    def test_should_do_crud_operations_with_curso(self):
        ...
        response = client.put(URL + CURSOS_URI + "/" + curso_id, data=json.dumps(curso_updated.dict()))
    ...
    def post_and_validate_curso(self, curso):
        response = client.post(url=URL + CURSOS_URI, data=json.dumps(curso.dict()))
    ...
    def create_curso(self):
        return CursoSchema(titulo=f"Testando 123 testando {random.randrange(0, 100)}", aulas=random.randrange(10, 300),
